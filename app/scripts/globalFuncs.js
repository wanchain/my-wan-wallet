...
globalFuncs.parityErrors = {
    "A (parity-01) A transaction with the same hash was already imported. It was probably already broadcast. To avoid duplicate transactions, check your address on [wanscan.org](https://wanscan.org) & wait 10 minutes before attempting to send again. [Learn More](https://myetherwallet.github.io/knowledge-base/transactions/transactions-not-showing-or-pending.html)\\. This means it was already broadcast. Please check your address on wanscan.org & wait 10 minutes before attempting to send again to avoid duplicate transactions. <a target='_blank' rel='noopener noreferrer' href='https://myetherwallet.github.io/knowledge-base/transactions/transactions-not-showing-or-pending.html'>Learn more</a>": 'PARITY_AlreadyImported',
...
globalFuncs.getParityMsg = function (str) {
    for (var reg in this.parityErrors) {
        if (this.parityErrors.hasOwnProperty(reg)) {
            const args = str.match('^' + reg + '$')
        if (args) {
                const key = this.parityErrors[reg]
...
globalFuncs.checkAndRedirectHTTPS = function () {
    var host = 'www.mywanwallet.com'
    var hostw = 'https://mywanwallet.com'
