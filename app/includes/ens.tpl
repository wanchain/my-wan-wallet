<main class="tab-pane ens-tab active" ng-if="globalService.currentTab==globalService.tabs.ens.id" ng-controller='ensCtrl' ng-cloak>
  ...
  <div class="block">
      ...
      <article class="cont-md">
        ...
        <p>
          ...
          This functionality is still experimental, use with care.
        </p>
      </article>
      <!-- / Title -->
  ...
  <div ng-hide="showENS()" class="alert alert-danger text-center">
    <p>
      The WNS is only available on the WAN chains. You are currently on the {{ajaxReq.type}} chain.
      <br />
      Please use the node switcher in the upper right corner to select "WAN".
    </p>
  </div>
  <!-- / IF NOT ENS CHAIN -->
      ...
      <div class="text-center" ng-click="wd = !wd">
        ...
        <h4>
          <span ng-show="objENS.status==ensModes.open">
            Do you want {{objENS.name}}.wan? Unlock your Wallet to Start an Auction
          </span>
          <span ng-show="objENS.status==ensModes.auction">
            Do you want {{objENS.name}}.wan? Unlock your Wallet to Place a Bid
          </span>
          <span ng-show="objENS.status==ensModes.reveal">
            Did you bid on {{objENS.name}}.wan? You must reveal your bid now.
          </span>
